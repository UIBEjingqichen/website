...
function previewFor(document, product) {
  return document.previewImages?.[0] || product.gallery?.[0]?.[0] || "applications/grid-substation-yard.jpeg";
}
...
function productOverviewSheet(product, page) {
  ...
  return sheet({
    ...
    page,
    body: `<div class="product-overview-grid"><div class="product-hero-image"><img src="${media(product.gallery?.[0]?.[0] || "applications/grid-substation-yard.jpeg")}" alt="${esc(productName(product))}"></div><div class="product-overview-copy"><p>${esc(product.description)}</p>${specGrid([
...
function projectSectionSheets(startPage) {
  const groups = [
    ["Renewable Energy & Storage", projects.filter((item) => ["Renewable Energy", "Energy Storage"].includes(item.application)).slice(0, 8), "applications/renewable-wind-solar-landscape.jpeg"],
    ["Industrial & Utility", projects.filter((item) => ["Industrial", "Utility Grid"].includes(item.application)).slice(0, 8), "applications/grid-substation-yard.jpeg"],
    ["Infrastructure & Public Projects", projects.filter((item) => item.application === "Infrastructure").slice(0, 8), "company/factory-campus-panorama.jpeg"],
    ["Selected International References", projects.filter((item) => item.country && item.country !== "China").slice(0, 8), "applications/renewable-wind-solar-landscape.jpeg"]
  ];
...
function coverSheet() {
  return `<section class="catalog-sheet catalog-cover" id="cover"><img class="cover-bg" src="${media("applications/grid-substation-yard.jpeg")}" alt="Power substation"><div class="cover-shade"></div><div class="cover-copy"><p>TIANYU ELECTRIC</p><h1>Transformer &<br>Substation Solutions</h1><span>Distribution · Power · Dry-Type · Prefabricated · Pad-Mounted</span><small>${esc(company.legalName)}</small></div><div class="cover-year">EXPORT PRODUCT CATALOG · 2026</div></section>`;
}
...
function companySheet(page) {
  return sheet({
    ...
    page,
    body: `<div class="company-grid"><div><p class="company-lead">${esc(company.productScope)}</p><p>${esc(company.groupBackground)}</p><p>${esc(company.manufacturingBase)}</p>${specGrid(companyStats.map((item) => [item.label, item.value]))}</div><img src="${media("company/factory-campus-panorama.jpeg")}" alt="Tianyu Electric factory campus"></div>`
  });
}
...
function backCover() {
  return `<section class="catalog-sheet back-cover"><img class="cover-bg" src="${media("company/factory-campus-panorama.jpeg")}" alt="Tianyu Electric"><div class="cover-shade"></div><div class="back-cover-copy"><p>TIANYU ELECTRIC</p><h2>Built around the project.<br>Backed by evidence.</h2><span>${esc(company.legalName)}</span><small>Technical Inquiry · Product Selection · Project Review</small><a href="index.html">Visit the Tianyu Electric website →</a></div></section>`;
}
