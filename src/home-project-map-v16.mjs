...
const projectConfig = [
  ...
  },
  {
    ...
    x: 53.2, y: 51.5,
    image: "catalog-desert-renewable-energy-base.png",
...
const selectedProjects = projectConfig.map(config => {
  ...
  if (!source) throw new Error(`Project library record not found: ${config.name}`);
  ...
  return {
    ...
    imageNote: "Tianyu media library"
  };
});
...
function attrs(p) {
  return [
    ...
    `data-image-note="${esc(p.imageNote)}"`,
...
function projectMapSection() {
  ...
  const pins = selectedProjects.map((p, index) => `<button class="ty16-pin${index === 0 ? " active" : ""}" type="button" aria-label="Review ${esc(p.title)}" style="left:${p.x}%;top:${p.y}%" data-ty16-pin ${attrs(p)}></button>`).join("");
  const cards = selectedProjects.map((p, index) => `<a class="ty16-feature-card${index === 0 ? " active" : ""}" href="${esc(p.href)}" data-ty16-card ${attrs(p)}><div class="ty16-feature-media"><img src="${esc(p.imagePath)}" alt="${esc(p.title)} reference visual" loading="lazy"></div><div class="ty16-feature-copy"><small>${esc(p.country || p.application)}</small><strong>${esc(p.title)}</strong><div class="ty16-feature-meta"><span>${esc(p.application || "Project reference")}</span><span>${esc(p.scale)}</span></div></div></a>`).join("");

  const html = `<section class="ty16-projects" id="global-project-references" data-ty16-projects><div class="ty16-shell"><div class="ty16-head"><div><p class="ty16-kicker">Global Project References</p><h2 class="ty16-title">Projects delivered across international markets</h2><p class="ty16-lead">Explore selected transformer and substation references by country and application. Project facts are drawn from Tianyu's existing project library.</p></div><a class="ty16-all-link" href="applications.html#projects">VIEW ALL PROJECTS →</a></div><div class="ty16-filters" aria-label="Filter project references">${filtersMarkup()}</div><div class="ty16-map-stage"><img class="ty16-world-base" src="${worldMapUrl}" alt="" aria-hidden="true">${pins}<span class="ty16-map-label">SELECT A PROJECT POINT</span><article class="ty16-float-card" aria-live="polite"><div class="ty16-float-media"><img src="${esc(first.imagePath)}" alt="${esc(first.title)} reference visual" data-ty16-image><span data-ty16-image-note>${esc(first.imageNote)}</span></div><div class="ty16-float-copy"><small class="ty16-float-country" data-ty16-country>${esc(first.country || "")}</small><h3 data-ty16-title>${esc(first.title)}</h3><p data-ty16-summary>${esc(first.summary)}</p><div class="ty16-facts"><div class="ty16-fact"><span>Application</span><strong data-ty16-application>${esc(first.application || "Project-specific")}</strong></div><div class="ty16-fact"><span>Industry</span><strong data-ty16-industry>${esc(first.industry || "Project-specific")}</strong></div><div class="ty16-fact"><span>Scale</span><strong data-ty16-scale>${esc(first.scale)}</strong></div><div class="ty16-fact"><span>Product</span><strong data-ty16-product>${esc(first.product)}</strong></div></div><a class="ty16-float-link" href="${esc(first.href)}" data-ty16-link>VIEW PROJECT →</a></div></article></div><div class="ty16-featured-head"><h3>Featured Projects</h3><span data-ty16-visible-count>4 selected references</span></div><div class="ty16-featured-grid">${cards}</div></div></section>`;
...
function ensureAssets() {
  ...
  for (const project of selectedProjects) {
    copyFile(
      path.join(root, "source-media", "applications", project.image),
      path.join(dist, "assets", "media", "applications", project.image)
    );
  }
}

function ensureLinks(html) {
  if (!html.includes("home-project-map-v16.css")) {
    html = html.replace("</head>", '  <link rel="stylesheet" href="assets/css/home-project-map-v16.css">\n</head>');
  }
  if (!html.includes("home-project-map-v16.js")) {
    html = html.replace("</body>", '  <script src="assets/js/home-project-map-v16.js"></script>\n</body>');
  }
  return html;
}
...
function patchHome() {
  ...
  if (/<section class="ty16-projects"[\s\S]*?<\/section>/.test(html)) {
    html = html.replace(/<section class="ty16-projects"[\s\S]*?<\/section>/, section);
  } else if (/<section class="ty-projects"[\s\S]*?<\/section>/.test(html)) {
    html = html.replace(/<section class="ty-projects"[\s\S]*?<\/section>/, section);
  } else {
...
function buildProjectPages() {
  ...
  ensureDir(dir);
  for (const project of selectedProjects) {
    fs.writeFileSync(path.join(dir, `${project.slug}.html`), detailPage(project), "utf8");
  }
}
...
function main() {
  ...
  patchHome();
  buildProjectPages();
  console.log(`Global project references rebuilt from ${selectedProjects.length} project-library records with price fields excluded.`);
}
