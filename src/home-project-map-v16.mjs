...
// These reference points are therefore country-level placement anchors used only to align pins to the
// Robinson world-map image. They must not be interpreted as exact project-site coordinates.
const projectConfig = [
  { name: "CEMIG 155 MW Photovoltaic Project", slug: "brazil-cemig-155mw-pv", lat: -14.2, lon: -51.9, image: "catalog-brazil-pv-export-project.png", summary: "155 MW photovoltaic project reference in Brazil, supplied with 29 high-capacity 35 kV prefabricated substations." },
  { name: "BCL Hattar Line 2 7200 TPD Cement Plant", slug: "pakistan-bcl-hattar-cement", lat: 30.4, lon: 69.3, image: "catalog-cement-plant-project.png", summary: "Cement-industry power-transformer reference recorded in Tianyu's project library." },
  { name: "Atlantic Industrial Park 132 kV Substation", slug: "nigeria-atlantic-132kv", lat: 9.1, lon: 8.7, image: "catalog-grid-110kv-substation-project.png", summary: "132 kV industrial-park substation reference recorded in Tianyu's project library." },
  { name: "Long Son Company Cement Grinding Project", slug: "vietnam-long-son-cement", lat: 16.0, lon: 108.0, image: "catalog-cement-plant-project.png", summary: "Cement grinding project recorded under Tianyu's high-voltage power-transformer references." },
  { name: "Methanol Dayyer Mobile Substation Export Project", slug: "uae-methanol-mobile-substation", lat: 24.3, lon: 54.4, image: "industrial-petrochemical-plant.jpeg", summary: "Mobile-substation export reference for a chemical-industry infrastructure project." },
  { name: "Sunshare Nambala 100 MW Solar Project", slug: "zambia-sunshare-nambala-solar", lat: -13.1, lon: 27.8, image: "utility-scale-solar-farm-aerial-01.jpeg", summary: "100 MW solar reference recorded under Tianyu's oil-immersed prefabricated substations." },
  { name: "CMOC Mining Area 500 MW Solar Project", slug: "drc-cmoc-mining-solar", lat: -4.0, lon: 21.8, image: "utility-scale-solar-farm-aerial-02.jpeg", summary: "500 MW mining / solar reference recorded under Tianyu's oil-immersed prefabricated substations." }
];
...
function interpolate(table, latitude) {
  const abs = Math.min(90, Math.abs(latitude));
  const low = Math.min(17, Math.floor(abs / 5));
  const t = (abs - low * 5) / 5;
  return table[low] + (table[low + 1] - table[low]) * t;
}
function projectRobinson(lat, lon) {
  const xScale = interpolate(robinsonX, lat);
  const yScale = interpolate(robinsonY, lat);
  return {
    x: +(50 + (lon / 180) * 50 * xScale).toFixed(2),
    y: +(50 - Math.sign(lat || 1) * yScale * 50).toFixed(2)
  };
}
...
function attrs(p) {
  return [
    `data-project-id="${esc(p.id)}"`, `data-country="${esc(p.country || "")}"`, `data-title="${esc(p.title)}"`,
    `data-summary="${esc(p.summary)}"`, `data-application="${esc(p.application || "")}"`, `data-industry="${esc(p.industry || "")}"`,
    `data-scale="${esc(p.scale)}"`, `data-product="${esc(p.product)}"`, `data-scope="${esc(p.scope || "")}"`,
    `data-image="${esc(p.imagePath)}"`, `data-href="${esc(p.href)}"`, `data-x="${p.x}"`, `data-y="${p.y}"`
  ].join(" ");
}

function filtersMarkup() {
  const filters = [["all", "All"], ["Renewable Energy", "Renewable"], ["Utility Grid", "Utility"], ["Industrial", "Industrial"], ["Infrastructure", "Infrastructure"]];
...
function buildProjectPages() {
  const dir = path.join(dist, "projects"); fs.rmSync(dir, { recursive: true, force: true }); ensureDir(dir);
...
function main() {
  ...
  ensureAssets(); patchHome(); buildProjectPages();
  console.log(`Global project references rebuilt from ${selectedProjects.length} records with Robinson-aligned country reference points.`);
}
main();
