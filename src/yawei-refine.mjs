...
function heroWave() {
  const slides = [
    ["hero-home-substation-grid.jpeg", "POWER TRANSFORMER & SUBSTATION SOLUTIONS", "Utility, renewable energy and industrial power projects."],
    ["products/power-transformer-220kv-240mva-ssz22.webp", "UP TO 220 kV POWER TRANSFORMER REFERENCES", "Model-specific test evidence from 110 kV to 220 kV systems."],
    ["company-factory-campus.jpeg", "FROM FACTORY TO PROJECT DELIVERY", "Engineering review, manufacturing, testing and document support in one workflow."]
  ];
...
function industrialLandscape() {
  const images = [
    ["company-factory-campus.jpeg", "Tianyu Electric factory campus"],
    ["hero-home-substation-grid.jpeg", "Substation and power equipment landscape"],
    ["case-renewable-energy-base.jpeg", "Renewable energy project landscape"],
    ["case-offshore-wind-project.jpeg", "Offshore renewable project landscape"],
    ["products/power-transformer-220kv-240mva-ssz22.webp", "220 kV power transformer"],
    ["products/dry-type-prefabricated-substation-01.webp", "Prefabricated substation"],
  ];
...
function productIndexMain() {
  const industryCards = [
    ["Power System", "hero-home-substation-grid.jpeg"],
    ["Industrial Field", "products/oil-distribution-transformer-02.webp"],
    ["Renewable Energy", "case-renewable-energy-base.jpeg"],
    ["Transportation", "case-offshore-wind-project.jpeg"]
  ];
  return `<section class="yw-product-banner" style="--banner:url('${media("products/power-transformer-220kv-240mva-ssz22.webp")}')"><div><p>PRODUCTS</p><h1>Transformer</h1></div></section>
...
function productMain(product) {
  ...
  const industryImages = ["hero-home-substation-grid.jpeg", "case-renewable-energy-base.jpeg", "products/oil-distribution-transformer-02.webp", "case-offshore-wind-project.jpeg"];
...
function newsMain() {
  return `<section class="yw-product-banner news-banner" style="--banner:url('${media("case-renewable-energy-base.jpeg")}')"><div><p>TIANYU ELECTRIC</p><h1>News & Knowledge</h1></div></section><section class="section yw-news-page"><div class="yw-index-title"><p>LATEST UPDATES</p><h2>Transformer Information News</h2></div><div class="yw-news-grid">${knowledgeHighlights.map((item, index) => `<a class="yw-news-card${index === 0 ? " featured" : ""}" href="${esc(item.href)}"><div class="yw-news-number">${String(index + 1).padStart(2, "0")}</div><p>${esc(item.category)}</p><h3>${esc(item.title)}</h3><span>${esc(item.summary)}</span><strong>READ MORE →</strong></a>`).join("")}</div></section>`;
}
