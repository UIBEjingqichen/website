...
function page({ title, description, active, canonical, content, depth = "" }) {
  const socialImage = siteUrl ? `${siteUrl}/assets/media/og-tianyu-electric.png` : "/assets/media/og-tianyu-electric.png";
...
function homePage() {
  ...
  return page({
    ...
    content: `
      ...
      <section class="section why-grid"><div><p class="eyebrow">Why Tianyu</p><h2>Evidence-led project collaboration</h2><p>Product selection, tested-model evidence, project references and manufacturing steps are connected through structured data so procurement and engineering teams can review the right scope quickly.</p><div class="proof-list"><span>Independent reports tied to exact models</span><span>Series capability kept separate from tested ratings</span><span>Reference drawings labeled as project-dependent</span><span>Missing assets hidden or clearly identified</span></div></div><img src="${media("company-factory-campus.jpeg")}" alt="Tianyu Electric factory campus" loading="lazy"></section>
...
function aboutPage() {
  return page({
    ...
    content: `
      <section class="page-image-hero"><img src="${media("company-factory-campus.jpeg")}" alt="Tianyu Electric factory campus"><div><p class="eyebrow">Company</p><h1>About Tianyu Electric</h1><p>${esc(company.tagline)}</p></div></section>
...
function manufacturingPage() {
  return page({
    ...
    content: `
      <section class="page-image-hero"><img src="${media("hero-home-substation-grid.jpeg")}" alt="Transformer factory and substation equipment"><div><p class="eyebrow">Manufacturing</p><h1>Manufacturing & Testing Capability</h1><p>A structured view of production, verification and quality-control stages.</p></div></section>
